...
/**
 * Created by zxsted on 15-10-26.
 *
 *
 * CNN2 用于对 CNN 的修改 ：
 *    1. 隔离导数与更新
 *    2. 添加全链接隐藏层和softmax输出层
 */
public class CNN2 {
     ...
     * 初始化网络
     */
    public CNN2(LayerBuilder layerBuilder, final int batchSize) {
        ...
        for (int i = 1; i < layers.size(); i++) {
            ...
            switch (layer.getType()) {
                ...
                case conv:
                    ...
                    // 设置 卷积核的梯度 和偏置
                    layer.initDKernel(frontMapNum);

                    layer.initDBias(frontMapNum);
                ...
                case output:
                    ...
                    // 初始化梯度
                    layer.initOutputDKerkel(frontMapNum, frontLayer.getMapSize());
                    layer.initBias(frontMapNum);
                ...
                case hidden:
                    // 初始化权重（）
                    // 这里分两中情况： 前面一层是 sampleLyaer
                    // 前面一层是 hiddenlayer  ：
                    // 情况的区分有builder 时 ，wsize 进行区分
                    layer.initHiddenLayerWeight(layer.getwSize());
                    // 初始化偏置
                    layer.initBarr(layer.getwSize().y);

                    // 初始化梯度
                    layer.initHiddenLayerDWeight(layer.getwSize());
                    layer.initDBarr(layer.getwSize().y);

                    // 初始化输出
                    layer.initAout(batchSize);
                    layer.initErr(batchSize);
                    break;
                case softmax:
                    // 初始化权重（）
                    layer.initHiddenLayerWeight(layer.getwSize());
    ...
    private void forward(Dataset.Record record) {
        ...
        for (int l = 1; l < layers.size(); l++) {
            ...
            switch (layer.getType()) {
                ...
                case hidden:
                    setHiddenOutput(layer,lastLayer);
                    break;
                case softmax:
                    setHiddenOutput(layer,lastLayer);
                    break;
        ...
        for (int l = layerNum - 2; l > 0; l--) {
            ...
            switch (layer.getType()) {
                ...
                case conv:
                    ...
                    break;
                case hidden:
                    setHiddenErrors(layer,nextLayer);
...
//                    printSize(lastMap);
    ...
    private void setHiddenOutput(final Layer layer,final Layer lastlayer)  {
        ...
        double[] aout = matrixOp(inMat,sigmod)[0];
    ...
    private void setHiddenErrors(final Layer layer,final Layer nextLayer) {
        ...
        if(nextLayer.getType() == Layer.LayerType.conv ) {
            ...
            double[] nexterr = nextLayer.getErr();
            double[][] nextWmat = nextLayer.getWmat(); //下一层的 权重矩阵

            double[][] nexterrMat = new double[1][];
            nexterrMat[0] = nexterr;

            // nextWmat size : n_in , n_out   nexterrMat: m:n_out
            // inerr: （m，n_out） * （n_out,n_in） => (m,n_in)
            double[][] inerr = dot(nexterrMat,T(nextWmat));

            double[] curAout = layer.getAout();

            double curAmat[][] = new double[1][];
            curAmat[0] = curAout;

            double[][] curErrMat =  matrixOp(curAmat, inerr, new Operator() {
                @Override
                public double process(double value) {
            ...
            double[] curErrArr = curErrMat[0];

            layer.setErr(curErrArr);
        }
    ...
    private double[][] gradient(Layer layer,Layer lastLayer) {

        double[][] inMat = new double[batchSize][];

        // 如果上一层的是采样层
        if (lastLayer.getType() == Layer.LayerType.samp) {

            int lastMapNum = lastLayer.getOutMapNum();         // 上一层的特征map的个数
            Size lastLayersize = lastLayer.getMapSize();       // 上一层的特征map的size

            double[][][][] maps = lastLayer.getMaps();

            // 将上一层的输出展平
            for (int i = 0 ; i < batchSize; i++) {
                for (int j =0 ; j < lastMapNum; j++)
                    for (int row = 0 ; row < lastLayersize.x; row ++)
                        for (int col = 0 ; col < lastLayersize.y; col++)
                            inMat[i][j*lastLayersize.x * lastLayersize.y + row * lastLayersize.y + col] = maps[i][j][row][col];
            }

        } else {
            inMat = lastLayer.getAouts();
        }

        double[][] err = layer.getErrs();   // 取出本批次的所有 err
        // inMat size: (batchsize,nin),layer.err:[batch,nout] ,wmat : (nin,nout)


        double[][] gradmat = dot(T(inMat), err);

        // 设置本层的梯度
        gradmat = matrixOp(gradmat, new Operator() {
            @Override
            public double process(double value) {
                return value / batchSize;
            }
        ...
        // gradmat size : （nin,nout）
        layer.setDw(gradmat);

        // 偏执的 梯度
        // 取出本层的所有的errs size:  (batchsize , nout)
        double[][] curErrMat = matrixOp(err, new Operator() {
            @Override
            public double process(double value) {
                return value / batchSize;
            }
        ...
        double barr[] = dimsum(curErrMat,1);

        layer.setDb(barr);

        double biasmat[][] = new double[1][];
        biasmat[0] = barr;
        gradmat = hstack(gradmat,biasmat);
    ...
    public double[][] updateWeights(Layer layer,Layer LastLayer,double[][] gradient) {

        double[][] wgrad = slice(gradient,0,gradient.length-1,0);
        double[][] bgrad = slice(gradient, gradient.length - 1, gradient.length, 0);

        double[][] wMat = layer.getWmat();
        double[] barr = layer.getBarr();
        double[][] bmat = new double[1][];
        bmat[0] = barr;
//        double[][] wbmat = hstack(wMat,bmat);

        //
        final double lrate = ALPHA;

        bmat = matrixOp(bmat, bgrad, null, null, new OperatorOnTwo() {
            @Override
        ...
        wMat = matrixOp(wMat,wgrad,multiply_lambda, multiply_alpha, plus);

        layer.setWmat(wMat);
        layer.setBarr(barr);

        double[][] newWbMat = hstack(wMat, bmat);
        return  newWbMat;
    }
    ...
    private void setSoftmaxOut (Layer layer,Layer lastLayer) {

        // wMat size: (n_in,n_out)
        double[][] wMat = layer.getWmat();
        // bias size = (1,n_out)
        double[] bias = layer.getBarr();

        double[] curfeatarr = lastLayer.getAout();

        double[][] curfeatval = new double[1][];

        curfeatval[0] = curfeatarr;

        // curfeatval size : (1,n_in)  wMat  size: (n_in,n_out)   bias: (1,n_out)
        // inMat  size : (1,out)
        double[][] inMat = addVec(dot(curfeatval, wMat), bias, 1);

        // aexpMat size : (1,n_out)
        double[][] aexpMat = matrixOp(inMat, exp);

        double[] rowsum = dimsum(aexpMat, 0);

        double[][] softMat = edivid(aexpMat, rowsum, 0);

        layer.setAout(softMat[0]);
    }


    /** 2. boolean backword(Record record )  负责计算本层的 error 逐个样本计算*/
    private boolean setSoftmaxerror(Dataset.Record record) {

        Layer softmaxLayer = layers.get(layerNum - 1);

        double[] softmaxout = softmaxLayer.getAout();
        int labelnum = softmaxout.length;     // 取出分类的个数

        double[] target = new double[labelnum];      // 标签向量
        double[] error = new double[labelnum];       // 误差向量

        int label = record.getLable().intValue(); // 当前记录的label
        target[label] = 1;

        double[] pre_val = softmaxLayer.getAout();

        for (int i = 0 ; i < labelnum; i++) {
            error[i] = target[i] -softmaxout[i];
        }
        softmaxLayer.setErr(error);

        return label == getMaxIndex(softmaxout);
    }


    /** 3. double[][] gradient（layer,lastlayer）  负责计算本层的梯度  ： 批次计算 **/
    public double[][] softmaxgradient(Layer layer,Layer lastLayer) {

        double[][] gradMat = null;

        // error mat size : (batchSize,n_out)
        double[][] error = layer.getErrs();
        // lastAout size : (batchSize,n_in)
        double[][] lastAout = lastLayer.getAouts();

        // gradMat size is: (n_in,n_out)
        gradMat = matrixOp(dot(T(lastAout), error), new Operator() {
            @Override
            public double process(double value) {
                return value / batchSize;
            }
        ...
        double[] bgrad = dimsum(matrixOp(error, new Operator() {
            @Override
            public double process(double value) {
                return value / batchSize;
            }
        ...
        double[][] bgmat = new double[1][];
        bgmat[0] = bgrad;

        return hstack(gradMat,bgmat);
    }


    /** 4. 跟新梯度 updateWeights（Layer layer ,Layer lastLayer ,double[][]）*/
//    public double[][] updateSoftmaxLayer(Layer layer ,Layer lastLayer ,double[][]) {
//
//        double[][] newWMat = null;
//
//
